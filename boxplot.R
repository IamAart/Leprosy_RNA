...
box_plot_libraries <- function(data, biotype) {
    ...
    plot <- ggplot(data) +
        ...
        labs(title = "Difference in auc score between RNA sequencing methods for all genes" , x = "RNA Sequencing Method", y = "Auc Score", color="RNA Sequencing Method") + 
        scale_color_viridis(
            discrete = TRUE,
    ...
    ggsave("./data/RF_analysis_plots/boxplot_libraries_all_genes.png", plot, width=12, height=6, dpi=300) 
}
